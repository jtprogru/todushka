...
Вариант **B** для звезды (чистый `View`). Scope урезать до двух
конкретных, тестируемых вещей: кольцо прогресса (BL-9) + звезда «today»
в Anytime (BL-10). Moon-иконку This Evening **не добавлять** — это BL-12,
неиспользуемый глиф сейчас был бы dead code. «Accent/whitespace tuning»
из BL-10 слишком расплывчат без критериев приёмки — в Deferred.

## Границы scope
- **Must-have (v1):** (1) кольцо ◯◔◑◕● у проектов в `project_list.go`;
  (2) жёлтая звезда у today-задач в списке Anytime в `viewList`.
- **Deferred (v2):** moon-иконка This Evening (→ BL-12); полировка
  глифов completed/cancelled; тюнинг акцентов/воздуха; агрегатное
  кольцо по area.
- **Needs spike:** нет.
...
- **Open Q1:** подтверждаете вариант B (предвычисление today-set)
  против A (`time.Now()` в рендере)?
- **Open Q2:** согласны урезать BL-10 до звезды (moon / полировка /
  воздух — в Deferred)?
