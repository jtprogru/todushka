...
- [x] **T-9** TUI screens — done.
  ...
  - Screen routing (list / quickentry / help) с переключателем `?`.
  - Editor screen в TUI v1 сведён к view-only + quick actions (complete/cancel/delete/pin) — полноценное редактирование полей (notes, dates, repeat) пока через CLI. Это отражено в README; включение в TUI v2.
- [x] **T-10** GATE — done.
...
- **Editor screen в TUI.** Реализован read-only + quick actions (`c`=complete, `x`=cancel, `d`=delete, `p`=pin to today). Полноценный form-редактор полей (notes/dates/repeat/checklist) отложен на v2; для богатого редактирования используется CLI (`todushka add` с флагами).
